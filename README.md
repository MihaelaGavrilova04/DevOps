...
Once code merges to main, the main-cicd.yml pipeline takes over:
  ...
  - Test Environment Deployment – Infrastructure stack provisioned in a local k3d cluster specifically for integration testing and validation.
...
**Infrastructure & Deployment** 
  ...
  *Kubernetes*
    ...
    - Ephemeral CI/CD Pipeline Environment
      As part of the main-cicd.yml pipeline in GitHub Actions, a clean,
      temporary k3d cluster is automatically created for integration testing and validation.
      This ensures that every change merged into the main branch is proven to deploy correctly in an isolated,
      production-like environment before it could be promoted further.
...
manage infrastructure as code, and deploy consistently using Kubernetes.
While built for learning, the pipeline uses real, industry-standard tools to provide practical, hands-on experience.
